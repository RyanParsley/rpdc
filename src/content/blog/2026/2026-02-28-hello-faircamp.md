...
This is the story of self-hosting a music portfolio in my homelab using
Faircamp. I very much doubt you are looking to set up exactly this project with
exactly this tech stack, but I suspect sharing my journey will be useful in one
way or another. All in all, the process is fairly simply. Perhaps, seeing that
will encourage you to try for yourself.
...
So, while this post is about how I'm hosting Faircamp, it's largely transferable
...
services running all configured via Coolify but I'm not going to use that today.
To use that would get in the way of the slick minimal deploy workflow that
Faircamp supports out of the box.

### So, how do I safely expose myself?

I don't want to make my local network any more public than I need to. So, instead
of opening a port on my router, I'm going to make use of Cloudflare's Zero Trust
Tunnel. At the time of writing, you can make use of this for free and it's a
very user friendly way to enable a local port to be accessible to the outside
...
move control of my domain name to Cloudflare. That went pretty smoothly because
it's a well documented popular choice. 

One gotcha that is easy to overlook is that the default CNAME for CloudCannon is
`site.cloudcannon.com` but it does not work with Cloudflare and that needs to be
changed to `orange-cloud.cloudcannon.com`.
...
So far, all of the services being hosted on this computer are by way of docker
containers. This means I didn't have any sort of web server installed directly
on the Pi yet. I'm not super familiar with Caddy, but I went with it on this
project over apache or nginx. It seems modern and light-weight and it did _just
work_ for me. Here is the entirety of my config (`/etc/caddy/Caddyfile`).
...
kind of suggests to me that you're using Apache. Services seem more appropriate
in `opt` and I am planning to set some up in Caddy soon. Considering I'm
planning to serve up some rust services soon, I'm going to standardize on `opt`
for all the things that Caddy cares about on this computer. 

Another thing to note if you're just getting started with Caddy is you want to
use the service for this. So, don't start Caddy on your server with `caddy
anything` you want to run `sudo systemctl enable caddy` that way the web server
is started every time your computer boots up.

## What does deployment look like?

Faircamp builds the site locally and even handles deployment with the following
arguments.
...
Here are my settings, yours will be different unless many fantastic coincidences
line up. 
